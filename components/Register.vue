...
<template>
  <v-dialog v-model="registerPrompt" @click:outside="close" width="auto">
    <v-card v-if="!isSuccess" elevation="0" color="accent" class="rounded-lg mx-auto" width="500px">
      ...
      <v-text-field v-model="data.taxNumber" outlined dense hide-details label="VKN/TCKN" class="rounded-lg ma-5"
        type="number" :error="button.errorState" @input="removeError" />
      <v-text-field v-model="data.email" outlined dense hide-details label="E-posta" class="rounded-lg ma-5"
        :error="button.errorState" @input="removeError" />
      <v-text-field v-model="data.phoneNumber" outlined dense label="Telefon numarası" class="rounded-lg ma-5"
